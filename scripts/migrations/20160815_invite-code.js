...
r.connect({ host: `localhost`, db: `phrase`, port: 28015 }, async (err, conn) => {
  await r.tableCreate(`inviteCodes`).run(conn)
  await r.table(`inviteCodes`).indexCreate(`userId`).run(conn)
  await r.table(`inviteCodes`).indexCreate(`code`).run(conn)
  await r.table(`inviteCodes`).indexWait(`code`).run(conn)
